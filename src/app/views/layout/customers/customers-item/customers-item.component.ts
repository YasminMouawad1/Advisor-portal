...
getUserById(){
  ...
  for(let i = 0 ; i< res.data.userDocuments.length ; i++){
    if(res.data.userDocuments[i].documnetType == 7)
       this.allContractImages[i] = res.data.userDocuments[i]
  }
   
   
     this.contractImage = this.allContractImages.filter(function(x) { return x !=undefined });
    ...
    if(this.contractImage.length > 0){
      this.imgSrc = this.contractImage[0].content;

      if(this.contractImage[0].activatorDetails != null){
  ...
  showImg(index: number) {
    this.imgSrc = this.contractImage[index].content;

    if(this.contractImage[index].activatorDetails != null){
      this.locationName = this.contractImage[index].activatorDetails.activatorLocation.name;
      this.mobileNum = this.contractImage[index].activatorDetails.mobileNumber;
      this.name = this.contractImage[index].activatorDetails.nameEn;
